...
public class EditorIntegrationManager {
    ...
    public CompletableFuture<Boolean> sendHeartbeatAsync(boolean online) {
        ...
        if (!plugin.isEnabled()) {
            try {
                ...
                // Fix for Issue #40: Better error handling and logging
                String errorMsg = ex.getMessage();
                if (errorMsg != null && errorMsg.contains("HTTP 500")) {
                    plugin.getLogger().severe("Heartbeat API returned HTTP 500 (Internal Server Error)");
                    plugin.getLogger().severe("This is usually a server-side issue. Check API server logs.");
                    plugin.getLogger().severe("Payload sent: " + buildHeartbeatPayload(online));
                } else {
                    plugin.getLogger().warning("Failed to send heartbeat: " + errorMsg);
                }
                future.completeExceptionally(ex);
            }
            return future;
            ...
            Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
                try {
                    ...
                    if (errorMsg != null && errorMsg.contains("HTTP 500")) {
                        plugin.getLogger().severe("Heartbeat API returned HTTP 500 (Internal Server Error)");
                        plugin.getLogger().severe("This is usually a server-side issue. Check API server logs.");
                        plugin.getLogger().severe("Payload sent: " + payload);
                        plugin.getLogger().severe("Server ID: " + serverId);
                    } else {
                        plugin.getLogger().warning("Failed to send heartbeat: " + errorMsg);
    ...
    private String postJson(String endpoint, String payload) throws IOException {
        ...
        try {
            ...
            // Fix for Issue #40: Better error handling for HTTP 500 errors
            if (code < 200 || code >= 300) {
                ...
                if (code >= 500) {
                    plugin.getLogger().severe("Server error (HTTP " + code + ") from heartbeat API endpoint: " + endpoint);
                    plugin.getLogger().severe("Response: " + response);
                    plugin.getLogger().severe("Payload: " + payload);
                } else if (code == 400) {
                    plugin.getLogger().warning("Bad request (HTTP 400) - check payload format: " + payload);
                } else if (code == 401) {
                    plugin.getLogger().warning("Unauthorized (HTTP 401) - check API key configuration");
                } else if (code == 404) {
                    plugin.getLogger().warning("Endpoint not found (HTTP 404): " + endpoint);
                }
            ...
            plugin.getLogger().warning("Could not connect to heartbeat API: " + e.getMessage());
            throw new IOException("Connection failed: " + e.getMessage(), e);
        } catch (java.net.SocketTimeoutException e) {
            plugin.getLogger().warning("Heartbeat API request timed out: " + e.getMessage());
            throw new IOException("Request timeout: " + e.getMessage(), e);
        } finally {
            if (connection != null) {
